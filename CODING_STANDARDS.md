...
function createCartState() {
  ...
  function add(item: CartItem) { items = [...items, item]; }
...
Default to none. Names carry the meaning. Add a comment only when the *why* is non-obvious: a hidden constraint, a workaround for a specific bug, an invariant that would surprise a reader six months from now.

Never write comments that restate the code or reference a ticket.

### Commit messages

Conventional Commits: `feat:`, `fix:`, `refactor:`, `docs:`, `chore:`, `test:`. Keep the subject under 72 chars. Body explains *why*, not *what*.
