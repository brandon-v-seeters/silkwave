...
<nav class="flex w-full items-center justify-between gap-3 md:hidden">
		<a href={resolve('/')} class="flex-shrink-0 font-serif text-lg text-foreground hover:opacity-80">
		silkwave
	</a>
			...
			>
				...
				<div class="flex min-h-0 flex-1 flex-col overflow-y-auto">
					...
					<div class="mt-auto border-b border-t border-border">
						...
						{#if genresExpanded}
							<ul class="border-t border-border/50 bg-accent/30 pb-2">
									{#each genres as genre (genre.href)}
									<li>
										<a
												href={resolve(genre.href as '/')}
					...
					<div class="border-t border-border p-4">
						<Button variant="primary" class="w-full" href={resolve('/upload')}>Upload Music</Button
						>
					</div>
				</div>
			</Dialog.Content>
...
>
	...
	<div class="flex flex-shrink-0 items-center gap-2">
		<Button variant="primary" size="sm" href={resolve('/upload')}>Upload Music</Button>
		<DarkModeToggle />
		<NavbarCart count={$cartCount} />
