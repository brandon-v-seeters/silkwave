...
<SelectPortal {...portalProps}>
	<SelectPrimitive.Content
		...
		class={cn(
			...
			'max-h-(--bits-select-content-available-height) origin-(--bits-select-content-transform-origin) relative z-50 min-w-[8rem]',
			'overflow-y-auto overflow-x-hidden rounded-lg border border-input shadow-md',
			// Animations
			'data-[state=open]:animate-in data-[state=closed]:animate-out',
			'data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0',
			'data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95',
			'data-[side=left]:slide-in-from-end-2 data-[side=bottom]:slide-in-from-top-2',
